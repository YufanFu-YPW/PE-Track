...
def sample_DanceTrack(dataset_root, deal_mod, sample_length, save_root):
    for mod in deal_mod:
        print(f'Start processing {mod} datasets')
        mkdirs(save_root)
        save_path = os.path.join(save_root,'dancetrack_' + mod + f'_{sample_length}.pkl')
        # print(save_path)
        # break
        ...
        for seq in seqs:
            ...
            space_conditions = get_space_conditions(new_gt, num_frames)
...
if __name__ == "__main__":
    sample_length = 5
    dataset_root = 'Datasets/DanceTrack'
    deal_mod = ['train', 'val']
    save_root = 'sample_datasets/DanceTrack'
    sample_DanceTrack(dataset_root, deal_mod, sample_length, save_root)
