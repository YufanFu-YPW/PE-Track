...
def sample_SportsMOT(dataset_root, deal_mod, sample_length, save_root):
    for mod in deal_mod:
        print(f'Start processing {mod} datasets')
        mkdirs(save_root)
        save_path = os.path.join(save_root,'sportsmot_' + mod + f'_{sample_length}.pkl')
        # print(save_path)
        # break
        ...
        print(f'SportsMOT {mod} datasets :  number of samples = {len(sample_dataset)}')
...
if __name__ == "__main__":
    sample_length = 5
    dataset_root = 'Datasets/sportsmot_publish/dataset'
    deal_mod = ['train', 'val']
    save_root = 'sample_datasets/SportsMOT'
    sample_SportsMOT(dataset_root, deal_mod, sample_length, save_root)
